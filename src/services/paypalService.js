...
export async function capturePayPalOrder(orderId, bookingId) {
  ...
  if (effectiveBookingId && data?.id && data?.status === "COMPLETED") {
    ...
    // Best-effort confirmation emails (booking + payment). Failures must not undo capture.
    try {
      const context = await loadBookingEmailContext(effectiveBookingId)
      if (!context?.email) {
        console.warn(`[paypal-email] skipped bookingId=${effectiveBookingId} reason=missing_email`)
      } else {
        console.log(`[paypal-email] start bookingId=${effectiveBookingId} to=${context.email}`)
        ...
        console.log(
          `[paypal-email] booking confirmation bookingId=${effectiveBookingId} success=${bookingEmail.success} messageId=${bookingEmail.messageId || "none"} error=${bookingEmail.error || "none"}`
        )
        console.log(
          `[paypal-email] payment confirmation bookingId=${effectiveBookingId} success=${paymentEmail.success} messageId=${paymentEmail.messageId || "none"} error=${paymentEmail.error || "none"}`
        )
        emailResults = {
      ...
      console.warn(
        "[paypal-email] confirmation send failed:",
